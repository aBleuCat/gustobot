...
async function handleHaiku(msg) {
    ...
    try {
        ...
        if (!targetMessage) {
            queueMessage({ channel: msg.channel, content: "I couldn't find a recent message of yours to check", reply: { message: msg, mention: true } });
            return;
        }
        ...
        if (result.isValid) {
            queueMessage({ channel: msg.channel, content: result.content, reply: { message: msg, mention: true } });
            return;
        }
        
        queueMessage({ channel: msg.channel, content: result.fixed, reply: { message: msg, mention: true } });
    } catch (e) {
