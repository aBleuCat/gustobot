...
interface SyllableAnalysis {
  ...
  isValid?: boolean;
  content?: string;
  counts?: Array<{ line: string; original: number; target?: number }>;
}
...
function getWordWithSyllables(targetSyllables: number): string | null {
...
function analyzeAndFixHaiku(text: string): SyllableAnalysis | null {
...
export async function handleHaiku(msg: Message): Promise<void> {
