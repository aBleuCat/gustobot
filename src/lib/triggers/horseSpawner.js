...
async function handleHorseSpawn(msg) {
    ...
    for (const [slug, data] of Object.entries(HORSE_VALUES)) {
        ...
        if (Math.floor(Math.random() * chance) === 0) {
            ...
            queueMessage({ channel: targetChan, content: `<@${msg.author.id}> ${prefix} **${displayName}**${decoration}!` });
    ...
    if (anySpawned) {
        ...
        if (Math.floor(Math.random() * config.COIN_CHANCE) === 0) {
            const minDrop = Number.isInteger(config.COIN_DROP_MIN) ? config.COIN_DROP_MIN : (config.COIN_DROP_SIZE || 1);
            const maxDrop = Number.isInteger(config.COIN_DROP_MAX) ? config.COIN_DROP_MAX : (config.COIN_DROP_SIZE || 5);
            const dropSize = Math.floor(Math.random() * (maxDrop - minDrop + 1)) + minDrop;

            inventory.horseCoins = (inventory.horseCoins || 0) + dropSize;
            
            queueMessage({ channel: targetChan, content: `<@${msg.author.id}> acquired **${dropSize} Horse Coins** 🪙!` });
        }
