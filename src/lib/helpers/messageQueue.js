...
function queueMessage({channel, content, reply = null}) {
	...
	if (queueLength > 60) {
		// Log to devlog if critical
		devLog(`Warning: ${queueLength} messages in queue`);
	} else if (queueLength > 20) {
		// Regular console log for warning
		console.warn(`[Warning] Message queue size reached ${queueLength}`);
	}
...
function isReady(message) {
	...
	const channelElapsed = now - (lastChannelSend.get(message.channel.id) || 0);
...
async function processQueue() {
	...
	while (messageQueue.length > 0) {
		...
		const index = messageQueue.findIndex((message) => isReady(message));
		...
		try {
			if (item.reply) {
				await item.channel.send({
					...
					reply: item.reply.mention
						? {messageReference: item.reply.message}
						: undefined,
				});
			} else {
