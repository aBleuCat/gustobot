...
async function conditionHorse(user, channel) {
	...
	if (hasLight && hasDark && !hasEqual) {
		const lightName =
			HORSE_VALUES.brightness_prosperity?.name ?? 'brightness_prosperity';
	...
	if (!hasJokery) {
		...
		if (totalWealth > 10_000_000) {
			const jokeryName =
				HORSE_VALUES.jokery_confusion?.name ?? 'Horse of Jokery and Confusion';
