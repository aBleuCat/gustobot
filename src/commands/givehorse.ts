...
export default {
    ...
    async autocomplete(interaction: AutocompleteInteraction): Promise<void> {
        ...
        if (inventory?.horses) {
            for (const [slug, count] of inventory.horses.entries()) {
                if (count > 0 && HORSE_VALUES[slug as keyof typeof HORSE_VALUES]) {
    ...
    async execute(interaction: ChatInputCommandInteraction): Promise<void> {
        ...
        const horseData: any = HORSE_VALUES[horseSlug as keyof typeof HORSE_VALUES];
        ...
        if ((interaction.client as any).logToModChannel) {
            await (interaction.client as any).logToModChannel(interaction.guild, `${interaction.user.tag} gave a ${horseDisplay} to ${targetUser!.tag}`);
        }
        if (interaction.channel && 'send' in interaction.channel) {
            await conditionHorse(receiverInv, interaction.channel as any).catch((e: any) => console.error(e));
        }
    }
};
