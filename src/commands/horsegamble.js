...
const ADMIN_IDS = new Set(['934290747623096381', '853658523786412063']);
...
function calculateCoinCostPerHorse(coinAmount) {
	return Math.max(
		1,
		Math.ceil((coinAmount / 50) * config.PROGRESSIVE_COIN_GAMBLE_TAX),
	);
}
...
function simulateBulkPass(slugsToGamble, virtualInv, costPerHorse) {
	...
	for (const slug of slugsToGamble) {
		...
		const effectivelossthresh =
			config.LOSS_THRESHOLD - Math.max(0, (startValue - 100) / 10);
...
function formatCycleLog(
	...
	const avgChange =
		totalGambled > 0 ? Math.round(netValueChange / totalGambled) : 0;
...
module.exports = {
	...
	async autocomplete(interaction) {
		try {
			...
			const inventory = normalizeHorseMap(
				await UserHorses.findOne({userId: interaction.user.id}).lean(),
			);
	...
	async execute(interaction) {
		...
		let inventory = isTest
			? null
			: normalizeHorseMap(
					await UserHorses.findOne({userId: interaction.user.id}),
				);
		devLog(
		...
		if (isCycleMode) {
			...
			for (let c = 1; c <= cycles; c++) {
				...
				if (bankAbove !== null) {
					...
					for (const slug of cycleHorses) {
						...
						if (value > bankAbove && (virtualInv.horses.get(slug) || 0) > 0) {
					...
					const newWholeCoins =
						Math.ceil(bankingCoinDebt) - totalCoinsSpentOnBanking;
					if (newWholeCoins > 0) {
			...
			const originalValue = [...originalMap.entries()].reduce(
				(sum, [s, cnt]) => sum + (HORSE_VALUES[s]?.value ?? 0) * cnt,
				0,
			);
			const finalActiveValue = [...finalActiveMap.entries()].reduce(
				(sum, [s, cnt]) => sum + (HORSE_VALUES[s]?.value ?? 0) * cnt,
				0,
			);
			...
			if (!isTest) {
				...
				for (const [slug, cnt] of gainedFromHouse.entries()) {
					houseInv.horses.set(
						slug,
						Math.max(0, (houseInv.horses.get(slug) || 0) - cnt),
					);
				}
			...
			if (!isTest) {
				const {newStreak: cycleStreak, awarded: cycleAwarded} = updateStreak(
					interaction.user.id,
					totalNetChange > 0,
				);
				if (cycleAwarded) {
			...
			for (const s of [...finalActiveMap.entries()]
				...
				.sort(
					(a, b) =>
						(HORSE_VALUES[b]?.value ?? 0) - (HORSE_VALUES[a]?.value ?? 0),
				))
			...
			if (cycleStreakMessage) finalLines.push(cycleStreakMessage);
			...
			const trueFinalLines =
				finalText.length > safeLength
					? [...finalLines.slice(0, 10), '... (full in attached file)'].concat(
							finalLines.slice(-10),
						)
					: finalLines;
			...
			if (!isTest)
				conditionHorse(inventory, interaction.channel).catch((error) =>
			...
			if (
				...
				if (victims.length > 0) {
					...
					for (const victim of victims) {
						...
						const effectivelossthresh =
							config.LOSS_THRESHOLD - Math.max(0, (victim.value - 100) / 10);
						if (fChange < effectivelossthresh) {
							const frenzyHouseInv = await getOrCreateInventory(
								UserHorses,
								HOUSE_USER_ID,
							);
							frenzyHouseInv.horses.set(
								victim.slug,
								(frenzyHouseInv.horses.get(victim.slug) || 0) + 1,
							);
							...
							inventory.horses.set(
								fClosest,
								(inventory.horses.get(fClosest) || 0) + 1,
							);
			...
			if (!isTest) {
				...
				if (commonTransfer > 0) {
					...
					if (actualDiff < 0) {
						houseInv.horses.set(
							COMMON_HORSE,
							(houseInv.horses.get(COMMON_HORSE) || 0) + commonTransfer,
						);
						devLog(
						...
						houseInv.horses.set(
							COMMON_HORSE,
							Math.max(0, houseCurrentCommon - commonTransfer),
						);
						devLog(
		...
		if (summary.length > safeLength) {
			await interaction.editReply({
				content: `Output too large, see attached file.` + bulkStreakMessage,
				files: [
					{
			...
			await interaction.editReply({content: summary + bulkStreakMessage});
		}
