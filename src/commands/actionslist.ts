import { SlashCommandBuilder, ActionRowBuilder, ButtonBuilder, ButtonStyle, EmbedBuilder, ChatInputCommandInteraction, MessageFlags } from 'discord.js';
...
export default {
    ...
    async execute(interaction: ChatInputCommandInteraction): Promise<void> {
        if (interaction.user.id !== '934290747623096381') {
            await interaction.reply({ content: "Only the great .i.exist can view these.", flags: MessageFlags.Ephemeral });
            return;
        }
        ...
        if (!actions.length) {
            await interaction.reply("I haven't learned any actions yet.");
            return;
        }
        ...
        const generateEmbed = (page: number) => {
            ...
            const embed = new EmbedBuilder()
                ...
                .setColor(0xFFA500)
                .setDescription(current.map((act: any) => `• **${act.trigger}** → ${act.response}`).join('\n'));
