...
function getSortedHorseList(inventory: any, sortDir: string = 'asc'): HorseItem[] {
  const list: HorseItem[] = [];
  for (const [slug, count] of inventory.horses.entries()) {
    if (count > 0 && (HORSE_VALUES as Record<string, any>)[slug]) {
      list.push({ slug, value: (HORSE_VALUES as Record<string, any>)[slug].value, count });
    }
  }
...
function coinValueForSlug(slug: string): number {
  const horseValue = (HORSE_VALUES as Record<string, any>)[slug].value;
  ...
  async execute(interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    if (isTopBottom) {
      ...
      const lines = [...sellMap.entries()]
        .sort(
          (a, b) =>
            ((HORSE_VALUES as Record<string, any>)[b[0]]?.value ?? 0) -
            ((HORSE_VALUES as Record<string, any>)[a[0]]?.value ?? 0)
        )
    ...
    if (!(HORSE_VALUES as Record<string, any>)[horseSlug]) {
