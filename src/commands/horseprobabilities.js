...
module.exports = {
	data: new SlashCommandBuilder()
		...
		.addSubcommand((subcommand) =>
			subcommand
				.setName('stats')
				.setDescription(
					'Show the spawn likelihood and message frequency per horse',
				),
		)
		.addSubcommand((subcommand) =>
			subcommand
				.setName('wheel')
				.setDescription(
					'Roll for a random horse (excludes non-spawning horses)',
				),
		),
	...
	async execute(interaction) {
		...
		if (sub === 'wheel') {
			...
			const spawnableKeys = horseKeys.filter(
				(key) => horsesData[key].spawn !== false,
			);
