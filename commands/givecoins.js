...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (target.id === interaction.user.id) {
            return interaction.reply({ content: `You can't give coins to yourself!`, flags: [MessageFlags.Ephemeral] });
        }
        if (target.bot) {
            return interaction.reply({ content: `You can't give coins to a bot!`, flags: [MessageFlags.Ephemeral] });
        }
        ...
        if (!sender || (sender.horseCoins || 0) < amount) {
            return interaction.reply({ content: `You don't have **${amount}** 🪙 to give!`, flags: [MessageFlags.Ephemeral] });
        }

        let receiver = await UserHorses.findOne({ userId: target.id });
        if (!receiver) receiver = new UserHorses({ userId: target.id, horses: new Map() });
        ...
        return interaction.reply(`<@${interaction.user.id}> gave **${amount}** 🪙 Horse Coin${amount !== 1 ? 's' : ''} to <@${target.id}>!`);
    }
};
