...
export async function handleHorseSpawn(msg) {
    ...
    if (!cache)
        cache = new MessageCache({ userId: msg.author.id, guildId: msg.guild.id });
    ...
    await MessageCache.findOneAndUpdate({ userId: msg.author.id, guildId: msg.guild.id }, {
        lastMessageTime: now,
        recentMessages: [msgText, ...(cache.recentMessages || [])].slice(0, config.RECENT_MSG_COUNT),
    }, { upsert: true });
    const targetChan = (await msg.guild.channels.fetch(hConfig.channelId).catch(() => null)) || msg.channel;
    let inventory = await UserHorses.findOne({ userId: msg.author.id });
    if (!inventory)
        inventory = new UserHorses({ userId: msg.author.id, horses: new Map() });
    ...
    for (const [slug, data] of Object.entries(HORSE_VALUES)) {
        ...
        if (Math.floor(Math.random() * chance) === 0) {
            ...
            await devLog(`[HORSE] ${msg.author.tag} spawned ${displayName} in guild ${msg.guild.name} (${msg.guild.id})!`);
            await targetChan
                .send(`<@${msg.author.id}> ${prefix} **${displayName}**${decoration}!`)
                .catch(() => { });
            if (data.link)
                await targetChan.send(data.link).catch(() => { });
    ...
    if (anySpawned) {
        ...
        if (Math.floor(Math.random() * config.COIN_CHANCE) === 0) {
            ...
            await targetChan.send(`<@${msg.author.id}> acquired **${dropSize} Horse Coins** 🪙!`);
            await inventory.save();
        }
    }
}
//# sourceMappingURL=horseSpawner.js.map