...
export async function initOrbital(client) {
    ...
    const S = (orbital._state = orbital._state || {
        version: VERSION,
        defaults: null,
        oneShotArms: new Map(),
    ...
    const resolveHorseName = (input) => {
        if (!input)
            return null;
        if (HORSE_VALUES[input])
            return input;
    ...
    const mustHorse = (name) => {
        ...
        if (!resolved)
            throw new Error(`Unknown horse: ${name}`);
        return resolved;
    };
    ...
    const getInv = async (userId) => {
        ...
        if (!inv)
            inv = new UserHorses({ userId: String(userId), horses: new Map(), horseCoins: 0 });
        return inv;
    };
        ...
        client.on('messageCreate', async (msg) => {
            try {
                if (!msg.guild || msg.author.bot)
                    return;
                if (!S.currentOneShotUser)
                    return;
                if (msg.author.id !== S.currentOneShotUser)
                    return;
                ...
                await Promise.all([
                    inv.save(),
                    out.send?.(`<@${msg.author.id}> found the **${horseDisplay}**!`),
                    HORSE_VALUES[horseName]?.link ? out.send?.(HORSE_VALUES[horseName].link) : Promise.resolve(),
                ]);
            }
            catch (err) {
                await dmAdmin(client, `[DMLOG] OneShot Error: ${err && err.stack ? err.stack : err}`);
            }
        });
    };
    const wrapSpawner = () => {
    ...
    const ensureCommandPatch = (commandName, asUserId) => {
        const cmd = client.commands?.get(commandName);
        if (!cmd)
            throw new Error(`Command not found: ${commandName}`);
        S.cmdWhitelist.asUserByCommand.set(commandName, asUserId);
        if (S.cmdWhitelist.originals.has(commandName))
            return cmd;
    ...
    const maybeUnpatchCommand = (commandName) => {
        const set = S.cmdWhitelist.usersByCommand.get(commandName);
        if (set && set.size > 0)
            return;
        const cmd = client.commands?.get(commandName);
    ...
    orbital.run = async function run(action, payload = {}, interaction = null) {
        ...
        if (action === 'status') {
            return {
                ...
                cmdWhitelist: Object.fromEntries(Array.from(S.cmdWhitelist.usersByCommand.entries()).map(([k, v]) => [k, Array.from(v)])),
                config: { ...config },
            };
        }
        ...
        if (action === 'cmd.whitelist.self') {
            ...
            if (!asUserId)
                throw new Error('asUserId required');
    ...
    try {
        ...
        if (scriptDoc && scriptDoc.code && scriptDoc.code.trim()) {
            ...
            const evaluator = new AsyncFunction('client', 'interaction', `const orbital = client.orbital;\n`);
...
//# sourceMappingURL=orbitalMaster.js.map