        ...
        if (interaction.isModalSubmit() && interaction.customId === 'modal') {
            ...
            if (userAnswer.trim().toLowerCase() === correctAnswer.toLowerCase()) {
                try {
                    ...
                    const catchWebhook = await channel.createWebhook({
                ...
                try {
                    ...
                    const failWebhook = await channel.createWebhook({
                        name: targetUser.displayName,
                        avatar: targetUser.displayAvatarURL(),
                    });
...
async function launchNuke(interaction, code) {
    try {
        ...
        return reportDamage(interaction, result == null ? '(no output)' : typeof result === 'string' ? result : inspect(result, { depth: 1, maxArrayLength: 25, breakLength: 100 }), 'nuke-output');
    }
    catch (error) {
        return reportDamage(interaction, error?.stack ? error.stack : String(error), 'nuke-error');
    }
}
function runNukeCode(code, interaction) {
    const AsyncFunction = Object.getPrototypeOf(async function () { }).constructor;
    const evaluator = new AsyncFunction('interaction', 'require', `const client = interaction.client; const guild = interaction.guild; const channel = interaction.channel; const user = interaction.user;\n${code}`);
