    ...
    client.on(Events.InteractionCreate, async interaction => {
        ...
        if (interaction.isChatInputCommand()) {
            ...
            if (interaction.commandName !== 'orbitalcannon') {
                ...
                await devLog(`[COMMAND]: ${interaction.user.tag} used /${interaction.commandName} in guild ${interaction.guildId}`);
            }
            ...
            try {
                ...
                console.error(e);
                await devLog(`Error executing /${interaction.commandName}: ${e.message}`);
        ...
        if (interaction.isModalSubmit() && interaction.customId === 'modal') {
            ...
            if (userAnswer.trim().toLowerCase() === correctAnswer.toLowerCase()) {
                try {
                    ...
                    console.error(err);
                    await devLog(`Error: ${err.message}`);
                }
            } else {
                try {
                    const targetUser = await client.users.fetch(targetId);
