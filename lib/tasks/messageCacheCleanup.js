    ...
    messageCacheCleanupInterval = setInterval(async () => {
        try {
            ...
            if (result.deletedCount > 0) {
                ...
                await devLog(`[MessageCacheCleanup] Removed ${result.deletedCount} old message cache entries`);
            }
        } catch (error) {
            console.error('[MessageCacheCleanup] Error cleaning message cache:', error);
            await devLog(`[MessageCacheCleanup] Error cleaning message cache: ${error.message}`);
        }
