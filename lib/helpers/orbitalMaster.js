...
function initOrbital(client) {
    ...
    // Enhanced: allow username or userId
    const toUserId = (payload, interaction) => {
        if (payload?.userId) return String(payload.userId);
        if (interaction?.user?.id) return String(interaction.user.id);
        if (payload?.username && interaction?.client) {
            // Try to resolve username to userId from guild members
            const username = payload.username.toLowerCase();
            let user = null;
            if (interaction.guild && interaction.guild.members) {
                user = interaction.guild.members.cache.find(m => m.user.username.toLowerCase() === username || m.displayName?.toLowerCase() === username);
            } else if (interaction.client && interaction.client.users) {
                user = interaction.client.users.cache.find(u => u.username.toLowerCase() === username);
            }
    ...
    function installOneShotListener() {
        ...
        client.on('messageCreate', async (msg) => {
            try {
                ...
                console.error('[Orbital] OneShot Error:', err);
            }
        });
    }
