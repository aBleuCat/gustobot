...
async function handleHorseSpawn(msg) {
    ...
    for (const [slug, data] of Object.entries(HORSE_VALUES)) {
        ...
        if (Math.floor(Math.random() * chance) === 0) {
            ...
            await targetChan.send(`<@${msg.author.id}> ${prefix} **${displayName}**${decoration}!`).catch(() => {});
            if (data.link) await targetChan.send(data.link).catch(() => {});

            await conditionHorse(inventory, targetChan);
        }
    }

    // horse coin spawn as per orbitalstone suggestion
    if (anySpawned || anySaved) {
