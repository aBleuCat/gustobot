...
async function handleHorseSpawn(msg) {
    console.log('[HORSE] SPAWN_COEFFICIENT:', config.SPAWN_COEFFICIENT);
    ...
    for (const [name, data] of Object.entries(HORSE_VALUES)) {
        ...
        const chance = Math.max(1, Math.floor(data.value * config.SPAWN_COEFFICIENT));
        console.log(`[HORSE] ${name}: chance=${chance}`); // temp debug
